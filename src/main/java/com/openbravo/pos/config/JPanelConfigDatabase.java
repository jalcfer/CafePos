...
//            jtxtDbSchema1.setEnabled(false);              
    ...
    private void jbtnReset1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtnReset1ActionPerformed
        ...
        jtxtDbSchema1.setText("jcfposdb");
...
*/
    ...
    private void jbtnCreateDBActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtnCreateDBActionPerformed
        try {
            ...
            if (isValid) {
                // postgresql
                if(jcboDBDriver.getSelectedIndex()==0){
                    Statement stmt = (Statement) connection.createStatement();
                    String SQL="CREATE DATABASE jcfposdb";

                    ResultSet resultSet = stmt.executeQuery("SELECT datname FROM pg_database WHERE datname = 'jcfposdb'");
                    if (!resultSet.next()) {
                        stmt.executeUpdate(SQL);
                    } 
                    resultSet.close();                   
                }else if(jcboDBDriver.getSelectedIndex()==1){ 
                    String SQL="CREATE DATABASE if not exists jcfposdb";
                    Statement stmt = (Statement) connection.createStatement();
                    stmt.executeUpdate(SQL);
                }
